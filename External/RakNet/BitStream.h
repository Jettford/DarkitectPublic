...
namespace RakNet
{
	...
	public:
		...
		inline void Write(ISerializable var) {
			var.Serialize(this);
		}
		...
		template <class templateType>
			std::pair<bool, templateType> Read();
...
#pragma warning(disable:4244)   // '=' : conversion from 'unsigned long' to 'unsigned short', possible loss of data
	...
	template <class templateType>
	inline std::pair<bool, templateType> BitStream::Read()
	{
#ifdef _MSC_VER
#pragma warning(disable:4127)   // conditional expression is constant
#endif
		templateType data;
		if (sizeof(data) == 1) {
			bool pairbool = ReadBits((unsigned char*)&data, sizeof(templateType) * 8, true);
			return std::make_pair(pairbool, data);
		}
		else
		{
#ifndef __BITSTREAM_NATIVE_END
